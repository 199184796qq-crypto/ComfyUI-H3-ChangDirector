...
def _export_signature(output_dir, filename_prefix, seg_idx):
    ...
    return _config_hash({
        ...
        "segment": int(seg_idx),
        "rule": "prefix_segment_timestamp_v1",
    })


def _export_segment_video(source_path, output_dir, filename_prefix, seg_idx,
    ...
    signature = _export_signature(output_dir, filename_prefix, seg_idx)
    ...
    prefix = _safe_export_prefix(filename_prefix)
    stem = "%s_%d_%s" % (prefix, int(seg_idx), datetime.now().strftime("%Y%m%d_%H_%M"))
...
def _select_h3_task(primary_model_kind, has_optional_fl2va, has_references,
    ...
    if primary_model_kind == "fl2va" and has_references:
        raise ValueError(
            "[H3导演台] 当前主模型识别为 FL2VA，但本段包含参考图、参考音频或参考视频；"
            "请把 Ref2VA 模型连接到 model，或移除本段参考素材。")
    if has_first_frame:
        if fl2va_available:
            return "fl2va"
        raise ValueError(
...
class H3DirectorStudio:
        ...
        return {
            "required": {
                ...
                "filename_prefix": ("STRING", {
                    "default": "ComfyUI",
                    "multiline": False,
                    "tooltip": "文件名规则：前缀_段号_年月日_时_分.mp4；同名时自动追加序号。"}),
                "外部文本目标段": ("INT", {
    ...
    def _run_segment(self, seg_idx, seg_cfg, shared_refs, model, fl2va_model, clip, vae, audio_vae,
        ...
        # 1) 组装画面条件。
        #    多参生视频沿用旧顺序：共享参考图 -> 上段尾帧 -> 本段 refs。
        #    文生 / 首帧 / 首尾帧 / 尾帧模式不混入角色图、上段尾帧或 Motion Context；
        #    它们只从本段 refs 按从左到右顺序取端点，避免不透明的条件叠加。
        ...
        tail_is_fl2v = (not endpoint_mode and tail_mode == "fl2v"
                         and seg_cfg.get("use_tail", True) and seg_idx > 1)
        if not endpoint_mode and seg_cfg.get("use_tail", True) and seg_idx > 1:
    ...
    def direct(self, model, clip, vae, audio_vae, width, height, 时长秒, steps,
        ...
        # 两套续接链路严格互斥。端点模式更不能混入上段尾帧或 Motion Context，
        # 否则用户在本段指定的首/尾帧会被额外条件覆盖。
        for _idx, _sc in enumerate(segments):
            if _generation_mode(_sc) != "multi_ref":
                _sc["use_tail"] = False
                _sc["motion_context"] = False
            elif _idx == 0:
                _sc["use_tail"] = False
                _sc["motion_context"] = False
            elif _sc.get("motion_context") is True:
        ...
        report.append("成片导出 | 目录 %s | 命名 %s_段号_年月日_时_分.mp4" % (
        ...
        for k, seg_cfg in enumerate(segments):
            ...
            if meta_ok and not seg_cfg.get("force"):
                report.append("段%d: 缓存命中，clip %d 已存在，跳过生成" % (seg_idx, seg_idx))
            else:
